...
use ruffle_core::events::{
    GamepadButton, KeyDescriptor, KeyLocation, LogicalKey, MouseButton, MouseWheelDelta, NamedKey,
    PhysicalKey, PlayerEvent, TextControlCode,
};
use ruffle_core::tag_utils::SwfMovie;
use ruffle_core::{FloatDuration, Player, StaticCallstack, ViewportDimensions};
    ...
    pub fn simulate_key_down(&self, code: String) {
        let _ = self.with_instance(|instance| {
            let _ = instance.with_core_mut(|core| {
                let physical_key = match code.as_str() {
                ...
                // Map physical key to logical key (what the game sees)
                let logical_key = match code.as_str() {
                    "Space" => LogicalKey::Character(' '),
                    ...
                    "ShiftLeft" | "ShiftRight" => LogicalKey::Named(NamedKey::Shift),
                    "ControlLeft" | "ControlRight" => LogicalKey::Named(NamedKey::Control),
                    other => {
                        // Letter keys: "KeyA" -> 'a', "Digit1" -> '1'
                        if other.starts_with("Key") && other.len() == 4 {
                            LogicalKey::Character(other.chars().nth(3).unwrap().to_ascii_lowercase())
                        } else if other.starts_with("Digit") && other.len() == 6 {
                            LogicalKey::Character(other.chars().nth(5).unwrap())
                        } else {
                            LogicalKey::Unknown
                        }
                    }
